...
coffee_data <- read.csv("data/arabica_data_cleaned_subsample.csv", na.strings = c("", "NA", " ", "n/a"))

# Select the features we want to focus on
...
# Calculate missing values and prepare for plotting
...
```{r, fig.width=4, fig.height=3.5, out.width="100%"}
...
\begin{minipage}[c]{0.48\textwidth}
```{r , fig.width=6, fig.height=4, echo=FALSE, out.width="100%"}
# Select only the numerical sensory columns 
...
top_features <- head(pfi_results$variable, 4)
...
```{r, plot-svm-pfi-ale, fig.width=8, fig.height=3.5}
pfi_plot_data <- head(pfi_results, 8)
pfi_plot_data$variable <- factor(
  pfi_plot_data$variable,
  levels = rev(pfi_plot_data$variable)
)

p_pfi <- ggplot(pfi_plot_data, aes(x = Importance, y = variable)) +
...
ale_combined_data <- do.call(
  rbind,
  lapply(names(ale_plots), function(feat) {
    d <- ale_plots[[feat]]$agr_profiles
    data.frame(
      Feature = feat,
      x = d$`_x_`,
      ale = d$`_yhat_`
...
```{r xgb-ale-calculation}
# Select top 4 features from PFI for ALE analysis
top_features <- head(pfi_results$variable, 4)
...
```{r xgb-pfi-pdp-combined, fig.width=8, fig.height=3.5}
# PFI plot
...
ale_combined_data <- do.call(
  rbind,
  lapply(names(ale_plots), function(feat) {
    ale_data <- ale_plots[[feat]]$agr_profiles
    data.frame(
      Feature = feat,
      x = ale_data$`_x_`,
      ale = ale_data$`_yhat_`
...
```{r shapviz-plots, fig.width=8, fig.height=4}
set.seed(SEED)

target_vals <- c(0.0, 10.0)
flavor_col <- which(colnames(X_train) == "Flavor")
wf_plots <- vector("list", length(target_vals))
titles <- c("Waterfall Plot (XGBoost) Lowest Flavor", "Waterfall Plot (XGBoost) Highest Flavor")

for (i in seq_along(target_vals)) {
  
  target_val_flav <- target_vals[i]
  obs_id <- which.min(abs(X_train[, flavor_col] - target_val_flav))
  ...
  wf_plots[[i]] <- sv_waterfall(sv, max_display = 8) +
    ggtitle(titles[i]) +
    theme_minimal(base_size = 7) +
    theme(
      plot.title = element_text(size = 7),
...
# Using the same observation selection logic as your XGBoost section target_vals <- c(0.0, 10.0)
library(patchwork)
svm_plots <- list()

titles <- c("Waterfall Plot (SVM) Lowest Flavor", "Waterfall Plot (SVM) Highest Flavor")

for (i in seq_along(target_vals)) {
  target_val <- target_vals[i]
  
  # Finding the observation in the test set to prove the model works on unseen data
  obs_id <- which.min(abs(train_data$Flavor - target_val))
  new_obs <- train_data[obs_id, , drop = FALSE]
  
  bd_svm <- predict_parts(explainer = explainer_svm, 
...
gridExtra::grid.arrange(wf_plots[[1]], wf_plots[[2]], ncol = 2)
