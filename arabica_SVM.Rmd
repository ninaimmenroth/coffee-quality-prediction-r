...
# Calculate Feature Importance PFI with loss_root_mean_square
...
Because we're using a linear model already (linear kernel of SVM), the PDP also works in that condition. However, if we would use random forest or any other non-linear model, PDP would'nt be a reliable xAI method.
...
plot(pdp_results) +
  ggtitle("Partial Dependence Plots for Coffee Quality", 
          subtitle = "Average effect of a feature on predicted Cupper Points") +
  theme_bw()
...
plot(ale_results) + 
  ...
  labs(title = "Accumulated Local Effects (ALE)",
       subtitle = "Safer for correlated features like Aroma and Flavor") +
  theme_bw()
```
