...
```{r xgb-load-data}
coffee_data <- read.csv("~/ML2 arabica-coffee-analayze/arabica_data_cleaned.csv", na.strings = c("", "NA"))
```
...
```{r, plot-svm-pfi-ale, fig.width=7.5, fig.height=2.6, out.width="50%"}
...
```{r xgb-pfi-pdp-combined, fig.width=6.5, fig.height=2.2, out.width="50%"}
# PFI plot
...
**For XGBoost:**
```{r shapviz-plots, fig.width=7, fig.height=3}
set.seed(SEED)
...
for (i in seq_along(target_vals)) {
  ...
  wf_plots[[i]] <- sv_waterfall(sv, max_display = 8) +
    ggtitle(paste("Target Flavor =", target_val_flav)) +
    theme_minimal(base_size = 7) +
    theme(
      plot.title = element_text(size = 7),
...
```{r svm-waterfall-plots, fig.width=11, fig.height=4, echo=FALSE}
...
for (i in seq_along(target_vals)) {
  ...
  svm_plots[[i]] <- plot(bd_svm, 
                         ...
                         digits = 2, 
                         shift_values = TRUE) + 
    ggtitle(paste("SVM Flavor Target =", target_val)) +
    theme_minimal(base_size = 7) + 
    theme(
