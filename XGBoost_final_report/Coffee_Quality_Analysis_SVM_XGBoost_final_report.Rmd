...
p_pdp <- ggplot(pdp_combined_data, aes(x = x, y = yhat)) +
  geom_line(color = "#2E86AB", linewidth = 0.8) +
  facet_wrap(~Feature, scales = "free_x", nrow = 1) +
  labs(x = "Feature Value", y = "Predicted Cupper Points", title = "Partial Dependence Plots") +
  theme_minimal(base_size = 7) +
  theme(strip.text = element_text(size = 6, face = "bold"), plot.title = element_text(size = 8))

grid.arrange(p_pfi, p_pdp, ncol = 2, widths = c(1, 1.5))
```
