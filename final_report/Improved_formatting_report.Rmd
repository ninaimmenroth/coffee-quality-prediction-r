...
**The source of the dataset**: https://github.com/jldbc/coffee-quality-database
...
```
**XGBoost** treats zero scores for Flavor or Aroma as critical indicators of poor quality. In its waterfall plot, the prediction drops sharply from 7.5 to 1.83. This reflects that the model recognizes the absence of key sensory features as a decisive factor, causing the quality score to collapse rather than decline gradually.

**The SVM model** is more conservative, with predictions remaining close to the mean. Even when sensory scores are zero, the predicted quality only decreases to 7.33. As a linear SVM, it applies a roughly uniform adjustment for each feature. Given that most coffees in the dataset are high-quality, the model tends to remain near its baseline prediction of 7.48.
