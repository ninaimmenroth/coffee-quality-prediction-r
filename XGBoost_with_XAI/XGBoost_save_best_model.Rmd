...
# Save models
xgb.save(model_xgb_s, "models/model_xgb_s.xgb")
xgb.save(model_xgb_e, "models/model_xgb_e.xgb")
xgb.save(model_xgb_f, "models/model_xgb_f.xgb")
...
```{r shap-individual, fig.cap="SHAP waterfall plot for a single prediction. Shows how each feature contributes to moving the prediction from the baseline (average) to the final predicted value.", fig.width=10, fig.height=6}
# Select an interesting observation (e.g., one with high prediction)
predictions_sample <- predict(model_xgb_f, xgb.DMatrix(X_sample))
selected_idx <- which.max(predictions_sample)

# Get SHAP values for this observation
shap_single <- shap_values$shaps[selected_idx, ]
baseline <- mean(y_train)
prediction_value <- predictions_sample[selected_idx]

# Prepare waterfall data
waterfall_data <- data.frame(
  feature = names(shap_single),
  shap_value = as.numeric(shap_single),
  stringsAsFactors = FALSE
)
waterfall_data <- waterfall_data[order(-abs(waterfall_data$shap_value)), ]
waterfall_data <- head(waterfall_data, 15)

# Create waterfall plot
waterfall_data$end <- baseline + cumsum(waterfall_data$shap_value)
waterfall_data$start <- c(baseline, head(waterfall_data$end, -1))
waterfall_data$feature <- factor(waterfall_data$feature, 
                                 levels = c("Baseline", waterfall_data$feature, "Prediction"))
...
ggplot(waterfall_data, aes(x = reorder(feature, -abs(shap_value)), 
  ...
  coord_flip() +
  labs(
    title = sprintf("SHAP Explanation for Observation %d", selected_idx),
    subtitle = sprintf("Baseline: %.2f → Prediction: %.2f", baseline, prediction_value),
    x = "Feature",
    y = "SHAP Value (contribution to prediction)"
  ) +
  theme_minimal() +
  theme(
...
```{r shap-insights, echo=FALSE, results='asis'}
# Get top positive and negative contributors
top_positive <- shap_importance$feature[1]
top_positive_impact <- shap_importance$mean_abs_shap[1]

# Calculate directional statistics
shap_directions <- data.frame(
  feature = colnames(shap_values$shaps),
  mean_shap = colMeans(shap_values$shaps),
  stringsAsFactors = FALSE
)
most_positive <- shap_directions[which.max(shap_directions$mean_shap), ]
most_negative <- shap_directions[which.min(shap_directions$mean_shap), ]

cat(sprintf("
**Notable patterns:**

1. **Most impactful feature**: %s (mean |SHAP| = %.4f)
   - This feature has the strongest average contribution to predictions
   - High importance indicates it's critical for accurate predictions

2. **Strongest positive driver**: %s (mean SHAP = %.4f)
   - On average, this feature pushes predictions higher
   - Consistently increases predicted Cupper Points

3. **Strongest negative driver**: %s (mean SHAP = %.4f)
   - On average, this feature pushes predictions lower
   - Consistently decreases predicted Cupper Points

4. **SHAP advantages**:
   - Provides both local (individual) and global (aggregate) explanations
   - Fair attribution based on game theory principles
   - Reveals complex interactions and non-linear relationships
   - Consistent with human intuition about feature contributions

5. **Consistency with other methods**:
   - Features with high SHAP importance typically also rank high in PFI
   - SHAP dependence plots often mirror ALE patterns but with more detail
   - Discrepancies can reveal interactions or context-dependent effects
", 
top_positive, top_positive_impact,
most_positive$feature, most_positive$mean_shap,
most_negative$feature, most_negative$mean_shap))
```
